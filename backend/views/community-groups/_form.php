...
<div class="card">
	...
	<div class="card-content collapse show">
		<div class="card-body">
			...
			<div class="row">
				<div class="col-md-6">
					<?= $form->field($model, 'CommunityGroupName')->textInput(['maxlength' => true]) ?>
				</div>
				<div class="col-md-6">
					<?= $form->field($model, 'FormationDate')->textInput(['type' => 'date']) ?>
				</div>			
			</div>
			...
			<div class="row">
				...
				<?= $form->field($model, 'CountyID')->dropDownList($counties, ['prompt' => 'Select...', 'class' => 'form-control',
										...
										$.post( "' . Yii::$app->urlManager->createUrl('projects/sub-counties?id=') . '"+$(this).val(), function( data ) {
											...
											$( "select#communitygroups-subcountyid" ).html( data );
										});
									']) ?>	
				</div>
				<div class="col-md-6">
					<?= $form->field($model, 'SubCountyID')->dropDownList($subCounties, ['prompt' => 'Select...', 'class' => 'form-control',
										...
										$.post( "' . Yii::$app->urlManager->createUrl('projects/wards?id=') . '"+$(this).val(), function( data ) {
											...
											$( "select#communitygroups-wardid" ).html( data );
										});
									']) ?>
				</div>			
			</div>
				...
				<div class="col-md-6">
					<?= $form->field($model, 'WardID')->dropDownList($wards, ['prompt' => 'Select...', 'class' => 'form-control',
												...
												$.post( "' . Yii::$app->urlManager->createUrl('projects/sub-locations?id=') . '"+$(this).val(), function( data ) {
													...
													$( "select#communitygroups-sublocationid" ).html( data );
												});
											']) ?>
				</div>
				<div class="col-md-6">
			...
			<div class="row">
				<div class="col-md-6">
					<?= $form->field($model, 'CommunityGroupStatusID')->dropDownList($communityGroupStatus, ['prompt'=>'Select']); ?>
				</div>
				<div class="col-md-6">
					<?= $form->field($model, 'DateDisbanded')->textInput(['type' => 'date']) ?>
				</div>			
			</div>
